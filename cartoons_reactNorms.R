...
qualRange<-seq(0,0.17,length=100)
alpha<-4
beta<--50
badgeFunc<-function(x,alpha,beta,maxBadge) {d<-maxBadge/(1+exp(alpha+beta*x))}
par(plt=posPlot())
plot(badgeFunc(qualRange,4,-50,1)~qualRange,type="l",cex.lab=1.5,cex.axis=2,lwd=3,
     col="red",ylim=c(0,1.1),xaxt="n",yaxt="n",ylab="",xlab="") #,col=i
