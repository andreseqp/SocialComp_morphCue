...
After every interaction they update the estimate of reward proportionally to the difference between their current estimate and the observed reward (prediction error $\delta$). Furthermore, they express different probabilities of retreating (attacking) depending on
...
approximation method based on radial basis functions [@sutton_Reinforcement_2018]. Specifically, we pick $c$ feature centers, which are evenly spaced values along the badge size interval ([0,1]). Each one of these features centers is associated with a weight for reward estimation and 
...
The response of each feature center dimishes as a gaussian function  with the distance between the feature center and the badge size of the partner (Fig. \ref{fig:model-struc} B, grey line). Formally the reward estimation 
...
of feature weigths associated with the actor, and the probability is obtained
