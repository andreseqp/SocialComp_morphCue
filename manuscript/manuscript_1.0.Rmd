...
par(new=FALSE)
color.bar.aeqp(paletteMeans1(100),min =min(colorbreaksQual),
               max = max(colorbreaksQual),nticks = 3,
               cex.tit = 0.9,title = "quality",
               numplotx = 20,numploty = 13,idplotx =9,idploty = 2)
...
The relationship is given by a threashold-like reaction norm, where the change from 
...
When we allow the evolution of the sender code ($\alpha$ and $\beta$ change subject to natural selection and genetic drift), and the badge size works as a handicap 
(i.e. the cost of the badge is inversely proportional to the quality of the individual), the sender code evolves to produce an honest signalFig \ref{fig:handicap}). 
