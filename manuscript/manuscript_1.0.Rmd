...
must be learned throughout and individual's life, thus it has the usual 
...
In an interaction each individual must 
...
along the badge size interval ([0,1]). This feature centres are the same for all individuals and stored in vector $\symbf{b}$ . Each one of these features centres 
...
reproduce proportionally to their total pay-off $w_i$, which is a sum of the 
...
norm parameters (Fig. \ref{fig:handicap}, A) show a set of 
...
with higher quality (Fig. \ref{fig:handicap} B). Receivers learn to react to such 
correlation, increasing the probability of retreat when facing 
individual of larger badges (Fig. \ref{fig:handicap} C). Hence, natural selection 
favours larger values of $\beta$, eventually leading to an evolutionary 
equilibrium where badge size is an honest signal of quality mediated
by the learned responses of receivers (Fig. \ref{fig:handicap} B.4 and C.4). 
The evolutionary trajectory portrayed in figure 
\ref{fig:handicap} A is not the only possible trajectory. If the slope of the 
...
quality of an individual (Fig. \ref{fig:noBadge}) . Eventually, the badge 
...
badge does not provide any quality information. 
...
the evolutionary dynamics. In the simulations presented in figure 
\ref{fig:handicap} and \ref{fig:noBadge}, 
individuals learned with high learning rate ($\alpha = 0.4$) and interacted 
repeatedly along their lifetime (on average 2000 interactions). When we reduce 
the number of interactions that individuals have over their lifetime to 300 
interactions on average, we see an drastic increase in the morphological and genetic
...
peaks in the distribution are not so clear-cut, but it's clear that there is an 
increase in the variance of the distribution. These changes in the 
distribution of the sender reaction norm parameters 
...
is not informative of their quality, while the third type shows 
intermediate badge sizes which are determined 
by the quality of the individual (Fig. \ref{fig:branching} B.3-4). 
...
```
The effect of a limited number of interactions on the amount of variation in 
the evolving parameters plays a role when the signal follows the handicap
...
of reaction norms. One with a steeper slope, meaning that it expresses 
...
learning, allows the coexistence of different communication strategies 
...
the badge size of the interacting partner. To asses if this initial 
...
we let naive individuals have a flat reaction norm with a 1) low (peaceful),
2) high (aggressive) probability of escalating a fight and 3) a probability 
...
that corresponds to peaceful naive individuals, is the only one where the  
distribution of values is split in different clusters. That is, the majority of 
...
values, and so the variation is driven mainly by among replicate differences. 
...
is to have a rare badge size. When the naive behaviour is peaceful, individual
...
the strength of frequency dependent selection diminishes. The diversification 
process presented here is fully dependent on the learning process and how this
