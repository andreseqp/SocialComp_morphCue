...
Communication systems relying on badges of status are often assumed 
to be cognitively less demanding than those involving individual 
recognition [@sheehan_There_2016] because in the latter, 
individuals need to store identity and associated information 
about each individual they interact with. In contrast, the response 
towards badges of status is often 
...
whether to aggressively engage in contest. An alternative view of 
badges of status is that individuals learn 
...
learning exists in all major vertebrate taxa, and in many invertebrates [@heyes_Simple_2012;@macphail_Brain_1982; @staddon_Adaptive_2016; 
...
mechanism whose underpinnings show interspecific variation [@enquist_Power_2016; @quinones_Reinforcement_2019], so presumably, it has been modified 
...
on the psychological and neurological mechanisms of associative learning [@sutton_Reinforcement_2018]. These set of algorithms allow the 
...
or as a trait for IR. We use the model to assess under what conditions 
...
which is drawnfrom a truncated normal distribution 
$N(\mu = 0.5,\sigma)$. An individual with quality $0$ has the lowest RHP, 
...
$B_i=1/(1+e^{\alpha_i-\beta_i Q_i})$; where are $\alpha_i$ and 
...
Unless, with a small probability, mutation changes the allelic value 
of the offspring. After birth individuals go through a round of 
viability selection. Individual specific survival probability is given by $s_i=1/(1+e^{-k_1-k_2(Q_i-B_i)})$. Where $k_1$ and $k_2$ are parameter 
...
extSimsDir<-paste0("M:/BadgeSims/",scenario1,"_")
...
extSimsDir<-paste0("M:/BadgeSims/",scenario2,"_")
listTest<-list.files(extSimsDir,full.names = TRUE)
indList<-grep("ind",listTest,value=TRUE)
# parameter values from project folder
paramName<-list.files(here("Simulations",paste0(scenario1,"_")))
...
quality of an individual. Eventually, the badge disappears from the population. 
