...
%DIF DEL .\manuscript_1.0.tex   Thu Nov 23 09:49:31 2023
%DIF ADD .\manuscript_2.0.tex   Tue Feb  6 10:41:50 2024
...
\hypersetup{
  pdftitle={The role of learning in the evolution of status signaling: a modeling approach},
  pdfkeywords={Biological signals, evolutionary branching, agent-based
modeling, animal communication, badges of status, handicap principle},
  hidelinks,
...
the signal works like a \DIFdelbegin \DIFdel{handicap }\DIFdelend \DIFaddbegin \DIFadd{`handicap' }\DIFaddend (Botero et al. 2010; Johnstone and
Norris 1993; Grafen 1990; Zahavi 1975). \DIFaddbegin \DIFadd{Throughout this manuscript, we
...
been questioned (Penn and Számadó 2020). }\DIFaddend Alternatively, the costs can
...
and Dale 2004). For example\DIFaddbegin \DIFadd{, }\DIFaddend in paper wasps, subordinate individuals
...
partner \DIFdelbegin \DIFdel{can depend }\DIFdelend \DIFaddbegin \DIFadd{depends }\DIFaddend on the size of the partner's badge, with the dependency
being determined by the focal's experiences acquired through a learning
process. \DIFdelbegin \DIFdel{We implement learning using the actor-critic
...
Individuals }\DIFdelend \DIFaddbegin \DIFadd{Individuals use previous experiences to }\DIFaddend estimate the reward
\DIFdelbegin \DIFdel{(pay-off) expected from interacting with
...
individuals of small badges in the future.
Given }\DIFdelend \DIFaddbegin \DIFadd{expected from encountering a partner with a particular badge size.
However, given }\DIFaddend that badge size is a real number between 0 and 1,
\DIFdelbegin \DIFdel{there are infinitely many badge sizes.
Thus, the reward estimation, as well as the probability of
retreating/attacking, }\DIFdelend \DIFaddbegin \DIFadd{previous experiences }\DIFaddend must be generalized across \DIFdelbegin \DIFdel{different values}\DIFdelend \DIFaddbegin \DIFadd{the badge size range}\DIFaddend . To
...
estimation \DIFaddbegin \DIFadd{(\(x_z\), \(z\) is an index that goes from 1 to \(c\)) }\DIFaddend and
tendency to play retreat in a given interaction \DIFdelbegin \DIFdel{. The
reward estimation and the }\DIFdelend \DIFaddbegin \DIFadd{(\(y_z\)). For each
badge size ecountered the reward estimate and the associated }\DIFaddend tendency to
retreat are calculated \DIFdelbegin \DIFdel{(in every
interaction) }\DIFdelend as the sum of the weights associated with each
...
\hat{R}_{ij} =  \sum_{z=1}^{c} x_z e\DIFdelbegin \DIFdel{^{(-\frac{|B_i-b_z|}{2\theta^2})} 
}\DIFdelend \DIFaddbegin \DIFadd{^{(-\frac{|B_j-b_z|}{2\theta^2})} 
...
\DIFaddbegin \DIFadd{The weights vectors for reward estimation (\(\symbf{x}\)) and tendency
...
trigger the ESS of the original }\emph{\DIFadd{hawk-dove}} \DIFadd{game and d) letting
...
\begin{equation}
  \DIFadd{\Delta x_z = \Alpha \delta e^{(-\frac{|B_j-b_z|}{2\theta^2})} \textrm{.}
  \label{eq:critic}
}\end{equation}

\DIFadd{Where \(\delta\) is the }\emph{\DIFadd{prediction error}}\DIFadd{, which is the difference
between the predicted and experienced reward, formally
}

\begin{equation}
  \DIFadd{\delta = R - \hat{R} \textrm{.} \label{eq:predError}
}\end{equation}

\DIFadd{The experienced reward (\(R\)) is the outcome from the }\emph{\DIFadd{hawk-dove}}
\DIFadd{game described earlier.
}

\DIFadd{Thus, the learning update depends on the speed of learning (\(A\)) and
...
\DIFadd{The weights for the probability of retreat (\(\symbf{z}\)) are updated
in a similar fashion according to
}

\begin{equation}
  \DIFadd{\Delta y_z = \Alpha \delta (1-\pi) e^{(-\frac{|B_j-b_z|}{2\theta^2})} \textrm{.}
  \label{eq:actor}
}\end{equation}

\DIFadd{where \(\pi\) is the probability of choosing the action chosen by the
individual. If an individual chooses to behave as a dove and avoid an
overt conflict,
}

\begin{equation}
  \DIFadd{\pi = q_{ij} = \frac{1}{1+e^{\sum_{z=1}^{c} y_z e^{-\frac{|B_i-b_z|}{2\theta^2}}}} \textrm{;}\label{eq:pi}
}\end{equation}

\DIFadd{if alternatively, the individual chooses to escalate the fight, \(\pi\)
corresponds to \(1-q_{ij}\). The combination of \(\pi\) and the
}\emph{\DIFadd{prediction error}} \DIFadd{(\(\delta\)) assures that behavioral policy
updates are carried increasing the probability of behaviours that
predict higher reward.
}

\DIFadd{This implementation corresponds to the }\emph{\DIFadd{actor-critic}} \DIFadd{approach from
reinforcement learning (RL) theory (Sutton and Barto 2018; Quiñones et
al. 2020; Leimar and McNamara 2019; Leimar 2021). In the
}\emph{\DIFadd{actor-critic}} \DIFadd{approach the agent updates its behavioral policy
(}\emph{\DIFadd{actor}}\DIFadd{) and in parallel its reward estimation (}\emph{\DIFadd{critic}}\DIFadd{). In
our model the actor update is implemented by equations \ref{eq:actor}
and \ref{eq:pi}; while the }\emph{\DIFadd{critic}} \DIFadd{update is implemented by
equation \ref{eq:critic}. Both of these updates include the
}\emph{\DIFadd{prediction error}} \DIFadd{(equation \ref{eq:predError}). The
}\emph{\DIFadd{prediction error}} \DIFadd{corresponds to the Rescorla-Wagner rule
...
\DIFaddend Individuals interact as the focal individual \(n\) times and the
interaction partner is chosen at random. Thus, the expected number of
interactions for each individual is \DIFdelbegin \DIFdel{\(n+\frac{n(N+1)}{N}\)}\DIFdelend \DIFaddbegin \DIFadd{\(2n\)}\DIFaddend . After the interaction round,
...
(\(w_0\)) and all the pay-offs obtained \DIFaddbegin \DIFadd{from interactions }\DIFaddend throughout
their life. Thus, the \DIFaddbegin \DIFadd{total fitness is composed of the viability
component (equation \ref{eq:viability}) and the total payoff collected
in the interaction round. Therefore, the }\DIFaddend combination of natural
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{manuscript_1.0_files/figure-latex/modelStruc-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{manuscript_2.0_files/figure-latex/modelStruc-1} 
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{manuscript_1.0_files/figure-latex/handicap-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{manuscript_2.0_files/figure-latex/handicap-1} 
\DIFaddendFL 

}

\caption{Evolution of the badge size as a handicap mediated by learning. Top panels (A.1 and A.2) show the evolutionary dynamics of the sender code. \DIFaddbeginFL \DIFaddFL{In y axis shows the trait value, in the x axis shows the generation time in thousands of generations. }\DIFaddendFL On the left (A.1), changes in the distribution of values for the intercept of the reaction norm ($\alpha$); on the right (A.2) changes in the distribution of the slope ($\beta$). Darker areas of the background correspond to values with high frequency, while orange lines in both panels show the mean of the distribution. \DIFdelbeginFL \DIFdelFL{X axis in the evolutionary dynamics are given in thousands of generations. }\DIFdelendFL Grey lines show the generation time corresponding to the panels below portraing the sender (B) and receiver code (C). In the bottom panels (C1-4), the learned reaction norms correspond to individuals in the population after the interaction round. Color scale indicates quality just as in Fig. \ref{fig:modelStruc}. \DIFaddbeginFL \DIFaddFL{The black dashed line shows the average reaction norm of the population. }\DIFaddendFL On the first half of the simulation (B.1 and B.2) the badge size evolves to its minimum value. In the second half, the slope takes positive values and reaction norms evolve to produce an honest signal of quality. Correspondingly, individuals learn to react to badge size by increasing the probability of retreating with increasing badge sizes\DIFaddbeginFL \DIFaddFL{.}\DIFaddendFL }\label{fig:handicap}
\end{figure}

\DIFdelbegin %DIFDELCMD < \hypertarget{badges-as-handicaps}{%
%DIFDELCMD < \subsubsection{Badges as Handicaps}\label{badges-as-handicaps}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{badges-as-handicaps-costly-signals}{%
...
involve a set of steps. First, the intercept of the \DIFaddbegin \DIFadd{logistic }\DIFaddend reaction
norm (\(\alpha\)) describing the relationship between individual quality
and badge size evolved to \DIFdelbegin \DIFdel{higher }\DIFdelend \DIFaddbegin \DIFadd{lower }\DIFaddend values, reducing the average badge size
...
badge does not provide any information about quality. \DIFaddbegin \DIFadd{This means that
...
\DIFdelbegin %DIFDELCMD < \hypertarget{the-evolution-of-polymorphism-mediated-by-learning}{%
%DIFDELCMD < \subsubsection{The evolution of polymorphism mediated by
%DIFDELCMD < learning}\label{the-evolution-of-polymorphism-mediated-by-learning}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We have described here two replicate simulations representative of the
...
\DIFaddbegin \hypertarget{the-evolution-of-polymorphism-mediated-by-learning-when-the-signal-is-cost-free}{%
...
\DIFaddend The amount of information that agents are able to collect via learning
...
distribution in the \DIFaddbegin \DIFadd{logistic }\DIFaddend intercept (\(\alpha\)) splits into a
...
of the \DIFaddbegin \DIFadd{logistic }\DIFaddend intercept (\(\alpha\)) in the population shows three
...
\DIFaddend In this simulation, the values of the slope in the informative group are
negative, which implies that badge size correlates negatively with
quality. \DIFdelbegin \DIFdel{Furthermore, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend receiver reaction norms developed through learning,
...
\ref{fig:branching}). \DIFaddbegin \DIFadd{However, given the absence of costs, the sign of
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{manuscript_1.0_files/figure-latex/branching-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{manuscript_2.0_files/figure-latex/branching-1} 
\DIFaddendFL 

}

\caption{The evolution of cheap signals. Portrait of the evolutionary dynamics of the sender code with snapsots of both sender and receiver codes just as in fig. \ref{fig:handicap}. The top panels (A) show changes in the distribution of values for $\alpha$ (A.1) and $\beta$ (A.2) along evolutionary time. \DIFaddbeginFL \DIFaddFL{The y axis shows the trait value, and  the x axis shows the generation time in thousands of generations. }\DIFaddendFL Panels below correspond to snapshots of the sender (B) and receiver codes (C). Generation time of the snapshots are indicated by the grey lines in the top panels. \DIFaddbeginFL \DIFaddFL{Color of the lines indicates the quality of the individual, as indicated in the color code in C.1. }\DIFaddendFL The \DIFaddbeginFL \DIFaddFL{dashed line represents the average reaction norm of the populations. The }\DIFaddendFL unimodal distribution with which $\alpha$ (A.1) starts the simulation, quickly turns into a bimondal distribution, and at about 10000 generations it splits further into three modes. These three peaks correspond to three type of reaction norms in panels B.1-3.}\label{fig:branching}
\end{figure}
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{manuscript_1.0_files/figure-latex/honestDiv-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{manuscript_2.0_files/figure-latex/honestDiv-1} 
...
\DIFdelbegin %DIFDELCMD < \hypertarget{the-peaceful-the-aggressive-and-the-clever}{%
%DIFDELCMD < \subsubsection{The peaceful, the aggressive and the
%DIFDELCMD < clever}\label{the-peaceful-the-aggressive-and-the-clever}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \hypertarget{the-peaceful-the-aggressive-and-the-clever-when-signals-are-cost-free}{%
...
clusters. That is, most of the variation occurs \DIFdelbegin \DIFdel{within clusters }\DIFdelend \DIFaddbegin \DIFadd{between clusters of each
replicate}\DIFaddend . In contrast, when naive individuals behave either
...
triggers \DIFdelbegin \DIFdel{negative-frequency dependent }\DIFdelend \DIFaddbegin \DIFadd{negative frequency-dependent }\DIFaddend selection and the evolution of
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{manuscript_1.0_files/figure-latex/startCond-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{manuscript_2.0_files/figure-latex/startCond-1} 
...
\DIFdelbegin \DIFdel{The action of natural selection and genetic drift on the learning
parameters do not override the conditions for the evolutionary branching
(Dieckmann and Law 1996) described above. }\DIFdelend Given how crucial naive behaviour is for the branching event described
earlier, we tested whether \DIFdelbegin \DIFdel{evolutionary processes could lead key parameters of }\DIFdelend \DIFaddbegin \DIFadd{the occurrence of branching was robust to the
...
populations. Thus, we developed an extension of the model where }\DIFaddend the
\DIFdelbegin \DIFdel{learning
process toward values that prevented the genetic variation to build up.
Figure \ref{fig:learnEvol} shows the
dynamics from a set of simulations
where we allow for the coevolution of the reaction norm as well as key
parameters of the learning module, namely the }\DIFdelend speed of learning (\DIFdelbegin \DIFdel{\(A\)}\DIFdelend \DIFaddbegin \DIFadd{\(\Alpha\)}\DIFaddend ) and the \DIFdelbegin \DIFdel{behavioural tendency expressed by individuals prior to learning}\DIFdelend \DIFaddbegin \DIFadd{initial value of the policy
...
parameters do not override the conditions for the evolutionary branching
(Dieckmann and Law 1996) described above}\DIFaddend . Top panels in figure
...
under the handicap principle\DIFaddbegin \DIFadd{, but it cannot without it }\DIFaddend (Grafen 1990;
Zahavi 1975). \DIFaddbegin \DIFadd{Learning is not sufficient to trigger the evolution of the
communication system. }\DIFaddend This conclusion is not different from classical
...
system. Namely\DIFaddbegin \DIFadd{, }\DIFaddend the polymorphism required a limit in the amount of
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{manuscript_1.0_files/figure-latex/noBadge-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{manuscript_2.0_files/figure-latex/noBadge-1} 
...
{\centering \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=1\linewidth]{manuscript_1.0_files/figure-latex/learnEvol-1} 
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=1\linewidth]{manuscript_2.0_files/figure-latex/learnEvol-1} 
...
%DIFDELCMD <   >{\centering\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.2143}}
%DIFDELCMD <   >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.7857}}@{}}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.1525}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.5593}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.2881}}@{}}
\DIFaddend \caption{Notation of the model parameters and variables\DIFaddbegin \DIFadd{. Values for the
parameters that are kept constant throughout all simulations are given
in the third column.}\DIFaddend }\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
Symbol
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Description
\end{minipage} \DIFaddbegin & \begin{minipage}[b]{\linewidth}\raggedright
\DIFadd{Value
}\end{minipage} \DIFaddend \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
...
\bottomrule\noalign{}
\endlastfoot
\(Q_i\) & Quality of individual \(i\) \DIFaddbegin & \DIFaddend \\
\(N\) & Population size \DIFaddbegin & \DIFadd{\(3000\) }\DIFaddend \\
\(n\) & Number of interactions as the focal individual \DIFaddbegin & \DIFaddend \\
\(\sigma\) & Standard deviation of the truncated normal distribution
from which quality is drawn \DIFaddbegin & \DIFadd{\(0.15\) }\DIFaddend \\
\(B_i\) & Badge size of individual \(i\) \DIFaddbegin & \DIFaddend \\
\(\alpha_i\) & Intercept of the badge size \DIFaddbegin \DIFadd{logistic }\DIFaddend reaction norm for
individual \(i\) \DIFaddbegin & \DIFaddend \\
\DIFaddbegin & & \\
\DIFaddend \(\beta_i\) & Slope of the badge size reaction norm for individual \(i\)
\DIFaddbegin & \DIFaddend \\
\DIFaddbegin & & \\
\DIFaddend \(\mu\) & Mutation rate \DIFaddbegin & \DIFadd{\(0.001\) }\DIFaddend \\
\(\sigma_\mu\) & Standard deviation of the normal distribution from
which quality is mutations are drawn \DIFaddbegin & \DIFadd{\(0.3\) }\DIFaddend \\
\(s_i\) & Survival probability of individual \(i\) \DIFaddbegin & \DIFaddend \\
\(k_1\) & Intercept of the survival function \DIFaddbegin & \DIFadd{\(3\) }\DIFaddend \\
\(k_2\) & Slope of the survival function \DIFaddbegin & \DIFaddend \\
\(V\) & Value of the contested resource \DIFaddbegin & \DIFadd{\(1\) }\DIFaddend \\
\(C\) & Cost of an escalated fight \DIFaddbegin & \DIFadd{\(1.5\) }\DIFaddend \\
\(p_{ij}\) & Probability that individual \(i\) wins an escalated fight
against individual \(j\) \DIFaddbegin & \DIFaddend \\
\(k_3\) & Parameter determining how important \DIFdelbegin \DIFdel{is quality }\DIFdelend \DIFaddbegin \DIFadd{quality is }\DIFaddend defining the
probability of wining \DIFaddbegin & \DIFadd{\(10\) }\DIFaddend \\
\(\delta\) & Prediction error \DIFaddbegin & \DIFaddend \\
\(A\) & Speed of learning \DIFaddbegin & \DIFaddend \\
\(c\) & Number of feature centres \DIFaddbegin & \DIFadd{\(6\) }\DIFaddend \\
\(b_z\) & badge size for feature centre \(z\) \DIFaddbegin & \DIFaddend \\
\(\hat{R}\) & Reward estimate \DIFaddbegin & \DIFaddend \\
\(x_z\) & weight of feature centre \(z\) on the reward estimation \DIFaddbegin & \DIFaddend \\
\DIFaddbegin & & \\
\DIFaddend \(\theta\) & width of the generalization function \DIFaddbegin & \DIFadd{\(0.01\) }\DIFaddend \\
...
\leavevmode\vadjust pre{\DIFaddbegin \hypertarget{ref-holland_Minireview_2016}{}}%DIF > 
\DIFadd{Holland, Peter C., and Felipe L. Schiffino. 2016. }{\DIFadd{``Mini-Review:
}{\DIFadd{Prediction}} \DIFadd{Errors, Attention and Associative Learning.''}}
\emph{\DIFadd{Neurobiology of Learning and Memory}} \DIFadd{131 (May): 207--15.
}\url{https://doi.org/10.1016/j.nlm.2016.02.014}\DIFadd{.
}

\leavevmode\vadjust \DIFadd{pre}{\DIFaddend \hypertarget{ref-johnstone_Recognition_1997}{}}%
...
\leavevmode\vadjust pre{\DIFaddbegin \hypertarget{ref-penn_Handicap_2020}{}}%DIF > 
\DIFadd{Penn, Dustin J., and Szabolcs Számadó. 2020. }{\DIFadd{``The }{\DIFadd{Handicap}}
{\DIFadd{Principle}}\DIFadd{: How an Erroneous Hypothesis Became a Scientific
Principle.''}} \emph{\DIFadd{Biological Reviews}} \DIFadd{95 (1): 267--90.
}\url{https://doi.org/10.1111/brv.12563}\DIFadd{.
}

\leavevmode\vadjust \DIFadd{pre}{\DIFaddend \hypertarget{ref-pretot_Factors_2016}{}}%
...
\url{https://doi.org/10.1126/science.224.4645.171}\DIFaddbegin \DIFadd{.
}

\leavevmode\vadjust \DIFadd{pre}{\hypertarget{ref-rescorla_Theory_1972}{}}%DIF > 
\DIFadd{Rescorla, R. A., and A. R. A. Wagner. 1972. }{\DIFadd{``A Theory of }{\DIFadd{Pavlovian}}
\DIFadd{Conditioning: Variations in the Effectiveness of Reinforcement and
Non-Reinforcement.''}} \DIFadd{In }\emph{\DIFadd{Classical Conditioning }{\DIFadd{II}}\DIFadd{: Current
Research and Theory}}\DIFadd{, edited by Abraham H. Black and William Frederick
Prokasy. New York: Appleton-Century-Crofts}\DIFaddend .
